...
An attribute that influences the dynamics must be either in the observation or out of the dynamics. Stochastic transitions keep the problem an MDP; unobserved state demotes it to a POMDP nobody chose, and a policy trained on such a state is learning from something that cannot explain its own transitions.
