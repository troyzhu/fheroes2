...
The formalism is worth two pages because it makes choices comparable. Once StarCraft, microRTS, NetHack, and a Heroes battle are all written as the same tuple, the differences become measurable rather than anecdotal, and a decision taken in one can be argued for or against in another.
...
A game environment is the tuple $(\mathcal{S}, \mathcal{A}, P, R, \gamma)$ plus an observation function $O$, and every environment design decision is a decision about one of those six objects.
...
Two properties of this mapping shape everything downstream. The decision boundary is a unit's turn rather than a round, so a five-stack battle produces about ten decisions per round rather than one. And the environment is nearly fully observed, since the interface reveals any unit's full statistics regardless of ownership, so the only genuinely hidden quantity is the future of the random generator.
...
| Heroes III battle ([[references/notes/ref-vcmi-gym\|vcmi-gym]]) | turn-based, alternating | 165 hex cells | up to 7 stacks | 2,312, masked | effectively full | yes | not published |
| microRTS ([[references/notes/ref-gym-microrts\|gym-µRTS]]) | real-time, simultaneous | 16×16 grid | dozens | factorized per cell | full or fogged by flag | mostly deterministic | very high |
| StarCraft II ([[references/notes/ref-pysc2\|PySC2]]) | real-time, simultaneous | large minimap | hundreds | huge, structured | fogged | yes | low |
| NetHack ([[references/notes/ref-nle\|NLE]]) | turn-based, single agent | 21×79 glyphs | one hero | ~100 discrete | fogged, partial | yes | high |
| Battle for Wesnoth ([[references/notes/ref-arlinbfw\|ARLinBfW]]) | turn-based, alternating | hex map | several | small discrete | full on the map | yes | low |
| Chess and Go | turn-based, alternating | 64 or 361 cells | fixed | hundreds to thousands | full | no | very high |
| Dota 2 ([[references/notes/ref-openai-five\|OpenAI Five]]) | real-time, simultaneous | large map | five heroes | large, parameterized | fogged | yes | low |

For what each of those codebases actually contains and where to look inside it, see [[references/repos]].
...
A competent scripted opponent already exists inside the game. It plays both sides for free, supplying both a demonstration source and an evaluation baseline without writing either. See [[concepts/teacher-coverage-and-behavior-cloning]].
...
Three environment properties matter more than they sound, independent of the agent. Determinism under a seed, so trajectories replay and regressions are detectable. Speed, so the learner rather than the simulator is the bottleneck. Engine-sourced legality, so the mask cannot disagree with what the simulator will accept. An environment missing any of the three can still train an agent, but debugging it becomes guesswork.
...
- [[START_HERE]] — the system as it stands, the build, and the current state.
- [[concepts/index|Concept primers]] — how each implemented mechanism works.
- [[references/summary|Literature synthesis]] — what the evidence establishes.
- [[references/repos|Repository orientation]] — the codebases behind the comparison.
