...
Measured here on 2026-08-04, ten seeds per arm on a calibrated matchup with everything else held fixed. Dropping the studentization is worth $+0.068 \pm 0.021$ in final win rate, 3.3 standard errors, which is the largest single effect in that comparison.

The other difference, whether the sample sits in its own baseline, cannot be measured here at all, and the reason is worth stating because it is not obvious. Within a group of $K$,

$$a^{\text{LOO}}_i = \frac{K}{K-1}\,a^{\text{DrGRPO}}_i$$

elementwise, so the two differ by a constant that does not depend on the sample. Advantage normalization then divides the batch by its own spread, which carries the same constant, and the two produce identical updates. The $O(1/K)$ bias that separates them on paper is exactly what the normalization absorbs, and the residual $+0.011 \pm 0.012$ observed between the arms comes from the advantage floor binding for one and not the other rather than from the baseline choice. Studentization survives normalization because it divides each group by a different number, which is why GRPO is a real third option and leave-one-out against Dr. GRPO is not.

This was re-derived here from two arms producing bit-identical runs, and it is Proposition 1 of [[../research/works/group-std-identity]], published five weeks earlier: mean centering is the leave-one-out advantage up to $G/(G-1)$, a constant their account absorbs into the learning rate and this pipeline absorbs exactly through the batch normalization. The same paper sharpens the studentization result. Under binary rewards a non-silent group's spread is bounded below, so GRPO's division is a bounded difficulty reweighting and the argument against it is about bias. Under this project's continuous margin-weighted reward the spread can be arbitrarily small without being zero, $1/\sigma$ is unbounded, and the division becomes noise amplification, which is what the $+0.068$ measurement shows and what the binary-reward literature has no reason to observe.
