...
Measured here on 2026-08-04, ten seeds per arm on a calibrated matchup with everything else held fixed. Dropping the studentization is worth $+0.068 \pm 0.021$ in final win rate, 3.3 standard errors, which is the largest single effect in that comparison. Including the sample in its own baseline, the other difference between leave-one-out and Dr. GRPO, is worth $+0.011 \pm 0.012$ and cannot be resolved, which is what an $O(1/K)$ bias at $K = 8$ should look like.

So the contested part of GRPO is the part that measurably hurts and the biased part is the part that does not matter, which is the Dr. GRPO argument holding in a setting nothing like the one it was made for. It is also the same failure as the amplification in [[training-design#An amplification in advantage normalization]], one level up: both divide by a spread that goes small when a group or a batch stops disagreeing, and both turn noise into a full-sized update. Full numbers in [[../archive/experiments/2026-08-03-training-runs#Advantage and trust region, ten seeds an arm]].
