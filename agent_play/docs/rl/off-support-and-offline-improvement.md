...
That test has now run: 9,018 soft-labeled decisions over 86 kept fresh matchups from both sides, temperature $\tau$ set to the median per-decision value spread (0.33) rather than guessed, trained beside the argmax twin on identical data. The soft arm leads six of nine suites, held-out 0.515 against 0.477 and commander extremes 0.990 against 0.969, while the twin keeps the Thunk ladder (0.812 against 0.760) and the defender mirrors.

A calibration note worth keeping: the prior-anchored target is inherently sharp here, 0.19 nats of average entropy, because both the prior and search are decisive, so the soft signal lives in the minority of rows where they disagree. Six of nine moving together is the coherence pattern the conventions ask for, at magnitudes each within single-suite noise, so the verdict was promising-not-adopted pending the multi-seed gate.
