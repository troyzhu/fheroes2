...
Search manufactures the counterfactual instead of estimating it: a rollout per candidate is a real measurement of an action the dataset never took, which is why rollout-scored search improves play here while every fitted estimator so far has not. Its per-candidate values are also the one dataset on which a learned evaluator could be trained without the off-support problem, since every labeled action was actually played, and the prior-anchored soft target of [[../research/works/mcts-regularized-policy-optimization]], $\bar\pi(a) \propto \pi_{\theta}(a) \exp(Q_{\text{search}}(a)/\tau)$, writing the soft-target temperature as $\tau$ since the notation contract reserves $\lambda$ for the advantage estimator,, keeps the distillation on support by construction. The collector records these values as of 2026-08-06 (`search_values` on decision records), and the owner proposed the softmax form independently before the paper was consulted.
...
Second the soft-target distillation from search values, the fifth answer above, piloted on a fresh both-sides collection. The pilot has run and validates the pipeline without answering the question: 1,183 soft-labeled decisions over 15 kept matchups trained beside a hard-label twin on identical data read as a wash, every suite difference inside noise with no coherent movement (`soft_distill.json`, `battery_soft.json`). Two reasons the test was underpowered are now design inputs. At $\tau = 0.5$ the targets averaged 0.249 nats of entropy, nearly one-hot, so the soft arm barely differed from its twin; the temperature needs to be set from the recorded value spread rather than guessed. And 1,183 soft rows against 266,375 hard ones is 0.4 percent of the corpus, so the arm was drowned regardless. The real test is a generation-scale collection, ten thousand plus soft-labeled decisions at a calibrated temperature, which is the recorded next step.

That test has now run: 9,018 soft-labeled decisions over 86 kept fresh matchups from both sides, temperature $\tau$ set to the median per-decision value spread (0.33) rather than guessed, trained beside the argmax twin on identical data. The soft arm leads seven of nine suites, held-out 0.515 against 0.477 and commander extremes 0.990 against 0.969, while the twin keeps the Thunk ladder (0.812 against 0.760) and the defender mirrors.

A calibration note worth keeping: the prior-anchored target is inherently sharp here, 0.18 nats of average entropy, because both the prior and search are decisive, so the soft signal lives in the minority of rows where they disagree. Seven of nine moving together is the coherence pattern the conventions ask for, at magnitudes each within single-suite noise, so the verdict was promising-not-adopted pending the multi-seed gate.

The gate ran the same evening and closed the thread: across three seeds the tilt does not replicate. Held-out reads $+0.04$, $+0.02$, $-0.02$ across seeds, the ladder favors the argmax twin on all three, and defender mirrors favor it decisively ($-0.13$, $-0.11$, $-0.04$). The seed-zero pattern was a roll, exactly the failure mode the multi-seed convention exists to catch, and the mechanism is the calibration note above: at 0.19 nats the soft target is the hard label plus a whisper, and the whisper does not help. The argmax label stands as the distillation target, the per-candidate recording stays (it costs nothing and feeds any future value-on-search-returns work), and reports `soft_distill_gen.json`, `battery_soft2.json` and their `_s1`/`_s2` replications carry the record.
