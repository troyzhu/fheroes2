...
What the same measurement does say plainly is that **9 of the 20 held-out matchups are already decided**, the engine winning at or above 0.9 or at or below 0.1 from both chairs, which caps the resolution of the project's headline suite whatever the cause. 

The first fix is now built, and building it answered a sharper question the owner asked next: whether a commander scales with the troop it leads. It does. The engine's strength is $(1 + \text{attack} \cdot 0.1 + \text{defense} \cdot 0.05)$ times a per-creature base, so an additive stat bonus is a multiplier that depends on the creature's own stats, and the same ten-attack ten-defense commander multiplies a Peasant stack by 2.30 and a Titan stack by 1.46, worth 1.6 times as much to the weakest creature as to the strongest. Matching commanders across two sides therefore does not equalize them unless the armies match too, so the budget has to price the commander per stack rather than treat it as a side-level constant.



The capability audit now records each creature's attack, defense and base strength, additively and with every pre-existing field byte-identical, which is what lets a Python-side pricing reproduce the engine's formula at all. `sample_budget_matchup` draws its commanders first and prices every stack under them, so the budget means the same thing on both sides and the budget ratio is the only intended asymmetry: over forty sampled matchups the commanded-strength ratio now sits at a median of 1.06. The second fix is still owed: the standing evaluation pool predates all of this, 43 of its 60 matchups carry a commander on one side only, and re-calibrating it would move every historical comparison, so it is a deliberate change to make once rather than a silent one.

Pricing the commander changes the sampled distribution beyond the parity it was aimed at, which the owner predicted and the measurement confirms. Drawing 400 matchups from the same seed stream under each pricing: the fraction whose commanded-strength ratio falls inside a balanced band, taking that as within a factor of about 1.4 either way, rises from 35 percent to 49, and the mean absolute log ratio falls from 0.67 to 0.59, so the sampler now produces meaningfully more matchups that are actually even. The side effect is that armies shrink, 21 creatures on average to 17, because a commanded side buys fewer creatures for the same budget.

That shrinkage is arguably correct rather than a defect, since a hero-led army should need fewer creatures to be equally strong, and the budget is denominated in strength. It is still a distributional change against every round collected before it: smaller armies mean shorter battles and fewer decisions per episode, so a corpus drawn under the new pricing is not interchangeable with one drawn under the old, and the budget range would have to be widened to hold army size fixed. Recorded rather than corrected, because which of the two the pool should hold constant, strength or size, is a modelling choice this page has not yet made.
