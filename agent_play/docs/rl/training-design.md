...
The longer direction was measured on 2026-08-11 and the row is now known to be wrong in a way the schedule was hiding. `CosineAnnealingLR` is armed `T_max=epochs`, so the rate reaches its floor exactly at the budget and a run annealed that way flattens at its own boundary: a flat tail says the schedule ended, not that the model converged. That is a property of this single-cycle configuration rather than of cosine schedules in general, and torch's own scheduler is periodic with period $2 T_{\max}$, so tying `T_max` to the epoch count is what keeps a run on the descending half at all. Re-armed to 100 epochs over three seeds, holdout agreement rises $0.851$ to $0.914$ while win rate falls $0.314$ to $0.284$, mean reward $0.122$ to $0.060$, and effective actions $1.66$ to $1.23$.

Paired within seed the long budget costs $-0.0301$ win rate at a standard error of $0.0112$ and $-0.0619$ reward at $0.0160$. Across fifteen arm-seeds, agreement correlates $-0.436$ with play, and that agreement is with `AI::BattlePlanner`, the engine the program is trying to beat, so its optimum is to match the engine rather than pass it. Best-agreement checkpointing therefore selects against play over this range, and saves a model some fifty epochs past the held-out loss minimum.

The other direction is answered too, and early stopping is not a lever either. At five, ten and fifteen epochs, each with the cosine re-armed to its own horizon, play is at or below the standing configuration on every column: five epochs matches on win rate at $+0.001$ while losing $-0.031$ reward at 4.3 standard errors and $-0.031$ strength margin at 20.1, so the 2026-08-07 eight-epoch hint does not survive a fuller metric block. Twenty-five epochs is the measured optimum in both directions. SGDR restarts settle what the damage is: restarting every 25 epochs over 100 buys the best agreement and the lowest held-out loss of any arm and produces the worst play of any arm, $-0.088$ win rate and $-0.211$ reward against the standing clone. The cost is the fitting, not the anneal's shape, and since the holdout is drawn from teacher episodes the signature is covariate shift rather than overfitting. See [[../archive/experiments/2026-08-11-distillation-budget-and-checkpoint-selection|the budget log]].
...
### Search leaves and the value question, measured 2026-08-06

The owner asked whether a value network could replace rollouts as the search's leaf evaluator, which is what would make multi-ply search affordable and the careful, low-loss lines expressible. The literature says the piece is standard: [[../research/works/alphazero]] and [[../research/works/muzero]] both train the value on the search's own play and jointly with the policy, [[../research/works/expert-iteration]] names the whole loop, and [[../research/works/mcts-regularized-policy-optimization]] adds that search improves on its prior only as far as the values it backs up are informative. The measurement says this project is not there yet, and says why.

A value fitted on the night's searched episodes explains 0.091 of return variance on searched holdout and $-0.050$ with a $+1.39$ bias on policy-played states, and one-ply search scored by it wins none of the Thunk fight that rollout search wins every time, whether the value comes from searched play or from the teacher-fitted critic.

Three causes, each fixable and none of them the estimator. The search corpus is win-filtered, so nearly every episode ends in victory and the value target is close to constant, which is the same outcome-variance degeneracy [[scenario-distribution]] measures for training: the filter that makes labels good for the policy makes them useless for a value, so the two need separate streams. The critic is a 193-parameter head on a frozen imitation trunk, where both papers co-train value and policy in one network. And a one-ply greedy value cannot beat a prior that already wins 0.81, so the value has to be good enough to rank moves before depth can pay.

#### The target and the trunk, isolated

The owner asked whether the value predicts per-state or only terminal outcomes, and the honest answer exposed a design flaw. It is per-state, every recorded decision carrying a target, but the target is $\gamma^{T-1-t}$ times the margin-weighted terminal reward, so an identical winning position is labeled 1.0 near the end and about 0.67 forty decisions out, and the survival term mixes a second objective into what should be a position judgment. [[../research/works/alphazero]] uses the undiscounted outcome instead, and the difference is measurable: refitting the same frozen-trunk head on plain $\pm 1$ outcomes raises held-out explained variance from 0.091 to about 0.19 on balanced data.

That still could not guide search, so the second suspect was isolated the same way. A dedicated value network, the same slot architecture but trained only to predict the outcome and sharing nothing with action imitation, reaches 0.610 explained variance, three times the probe and six times the original critic.

The retest then taught the harder lesson. A first run read 1.000 on the Thunk fight and looked like the breakthrough, and it was a lottery ticket: the probe script retrained its network at import time with an unseeded initialization and overwrote the checkpoint on every run, so the winning weights were one draw among many and are gone. Every reproducible draw since reads 0.000, including a seeded refit on the full 270,000-sample bestiary corpus that reaches 0.856 explained variance, the best value this project has ever fitted.

The verdict is sharper than a failure: state-level explained variance, even high, does not transfer to ranking successor states one action apart, which is what a leaf evaluator is for. That is the move-level version of the lesson the cloning seed noise taught at the checkpoint level, and it points where the literature does, at values trained on search returns and at move-level targets rather than state regression ([[../research/works/muzero]], [[../research/works/mcts-regularized-policy-optimization]]). Rollout-scored search stays the only search this project trusts. The frozen imitation trunk was the binding constraint, exactly as the co-training in [[../research/works/muzero]] implies, and this project's freeze was adopted for a different and still valid reason, protecting the policy from catastrophic forgetting. The resolution is therefore a separate value network rather than an unfrozen shared one, which is now the recorded design for any leaf evaluator, alongside unfiltered data with real outcome variance.

One defect of this project's own making is recorded because it distorted the first reading: the critic is fitted on the return of whoever acts at a state, so a successor where the opponent acts must be negated, and the first probe scored it unsigned. Fixing the sign left the verdict unchanged, which is what makes the three causes above the real ones. Generalized advantage estimation ([[../research/works/gae]]) is not the lever here either: with terminal-only rewards over 5 to 40 decisions, Monte-Carlo returns are already unbiased, and $\lambda$-returns need a trustworthy value to bootstrap from, which is the thing missing.

### The night of generations, 2026-08-06

The eight-hour autonomous block ran fresh-distribution search teaching to its plateau, recorded in [[../archive/experiments/2026-08-06-night-block-search-generations]]. Generation one, 9,219 root-PUCT labels over 83 win-filtered fresh matchups from both sides, produced the best single checkpoint measured on the untouched Thunk ladder, `policy_gen1` with top-rung reads of 0.54 to 0.79 and everything else at parity with the previous champion; the closing seed replication put the recipe's expected top rung near 0.43 with seed noise around 0.10, so the artifact outperforms its own recipe's mean and single-roll rung comparisons carry that noise, a convention now recorded in the experiments README. Every attempt to climb past it traded axes instead: a second generation from the improved policy, its defender-only split, triple weight on the winning labels, and deeper 48-simulation labels all moved some suites up and the peak down.

Three findings promote. Fresh-distribution win-filtered search labels are worth one large step and then the supervised mixture saturates at this corpus and network, every further label set buying its axes at the price of others. The win filter matters because roughly half the raw distribution is winnable by nobody, and the fresh-sampled suite reads 0.37 to 0.39 for every checkpoint from clone v4 onward, so improvement there is not a meaningful target. And the mixture dial is monotone-then-costly universally, even on a label set's own weight. The recorded escalations past the plateau are architectural (pooling against concatenation, and the ability-record consumer), not more labels.
...
#### The behavior value, measured where it would be spent

The owner's 2026-08-05 review asked what is wrong with the value model, and the answer is that it is a behavior value in the offline-RL sense, accurate only on the distribution that fitted it. Refitted at `obs_encoding_v3` on the full current corpus (`critic_calibration.py`), it explains 0.302 of held-out return variance on teacher play, and the fixture-era $+0.835$ was a narrow-era, episode-split figure that does not survive diversity. On student-played states, the DAgger collections, explained variance is $-0.131$, worse than predicting the mean, with a systematic optimistic bias of $+0.32$: the critic predicts teacher-grade outcomes for situations only the student reaches. That is the extrapolation error [[../research/works/bcq-extrapolation]] names, and it is the mechanism by which [[../research/works/alphastar-unplugged]] saw search at training time collapse the policy, since any operator that maximizes over the estimate walks into its optimism.

Three consequences bind consumers of this critic. As a GAE baseline it misleads most exactly where an exploring policy goes, which plausibly contributed to the measured erosion of unanchored PPO. As a search leaf evaluator it would inflate off-data branches, so the search probe scores branches by full engine rollouts instead, immune by construction because rollouts query the engine rather than the estimate. And as a shaping potential it stays formally policy-invariant, since the Ng-Harada-Russell guarantee needs no accuracy, but its informativeness inverts off-distribution. The bootstrapping caveat of [[../research/works/double-q-overestimation]] is currently absent, Monte-Carlo targets involve no max, and becomes live the moment a TD critic or value-maximizing head enters.
