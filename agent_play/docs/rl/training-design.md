...
The retest closes the loop: one-ply search scored by that network wins the Thunk fight every time, where both frozen-trunk values scored 0.00, so the leaf-evaluator requirements are demonstrated rather than argued and the multi-ply door is open. Its surviving strength, 0.336 against rollout search's 0.443, is consistent with a value that knows outcomes and not preservation. The frozen imitation trunk was the binding constraint, exactly as the co-training in [[../research/works/muzero]] implies, and this project's freeze was adopted for a different and still valid reason, protecting the policy from catastrophic forgetting. The resolution is therefore a separate value network rather than an unfrozen shared one, which is now the recorded design for any leaf evaluator, alongside unfiltered data with real outcome variance.
