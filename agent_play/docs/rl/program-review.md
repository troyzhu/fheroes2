...
The 0.963 reading that anchors the agent regime rests on 80 episodes: twenty matchups at four battlefields, one checkpoint, one simulation count, one exploration constant, one reward margin, and code that predates the tie-break fix. Its quoted error is a standard error over twenty matchup means, so a single matchup slipping from four wins to two moves the headline by about the width of the bar, and nine of those twenty matchups are already decided in the engine's own hands, which leaves the crossing carried by roughly eleven matchups at four episodes each. The built-in AI column beside it was measured at six times the episode count, so the comparison is power-asymmetric in the engine's favour.
