...
## The experiment, run 2026-08-07

Mean pooling was measured exactly as designed: three seeds beside a capacity control (the mean trunk widened back toward the concat count), on the champion planes mixture, against the concat baseline's same-seed columns. The result splits the hypothesis in half.

Fidelity collapses: cloning agreement falls from concat's $0.8525 \pm 0.002$ to $0.718$ for mean and $0.755$ for the widened mean, ten to thirteen points, far outside seed noise. The mechanism is visible in the architecture: averaging ten embeddings into one dilutes exactly the actor-centric information the action space needs, since which stack is acting and where every enemy stands must survive the pool for a cell-addressed action to be chosen, and a plain mean gives the active stack one tenth of the vote. Invariance was bought at the price of addressability.

Play does not collapse, and that is the finding. Held-out reads $0.522 \pm 0.026$ against concat's $0.480 \pm 0.009$, commander extremes $0.986$ against $0.955$, the ladder at parity, with the defender suites a few points down; a policy imitating the teacher thirteen points worse plays no worse and in places better, on 42 percent fewer parameters. Agreement and play decouple here just as the plateau arms showed from the other direction.

The verdict is measured-not-adopted: concat stays, because distillation pipelines live on fidelity and the defender suites paid. The recorded next arm, if pooling returns, is attention with the active stack as query, which keeps permutation invariance while restoring the addressability a mean destroys. Reports `pooling_s{0,1,2}.json`, `battery_pooling_s{0,1,2}.json`.
