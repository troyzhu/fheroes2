...
- `src/fheroes2/agent/`, the entry-point-free library, compiled into the normal executable by both build systems (spec §6.1 sanctions this) but unreachable from game code: no game translation unit includes an `agent_*` header. Launch smoke tests of both binaries pass. The two added since Milestone 3 are `agent_observation.{h,cpp}`, which serializes the per-decision board, and `agent_external_controller.{h,cpp}`, which blocks on a callback so a Python policy can answer a decision.
...
- `src/fheroes2/ai/ai_battle.{h,cpp}` (teacher probe, 2026-08-05), one public `queryUnitTurn` forwarding to the private `planUnitTurn`, so the agent-side probe can ask what the planner would do without the attacking-side turn-limit bookkeeping `BattleTurn` adds. No behavior change for any existing caller. Proof of inertness: all gate digests unchanged, and `planner_query.py` shows 100 paired episodes with terminal digests identical whether or not the probe runs.
...
| `src/fheroes2/agent/agent_trajectory.{h,cpp}` | `DecisionRecord`/`EpisodeRecording`, SHA-256 decision-stream digest (`agent_decisions_v0`), JSONL `TrajectoryWriter` (`agent_passive_v1`: per-unit terminal state and side summaries, both strength pricings, no wall-clock fields) | §15 subset | `verify_m2.sh` byte-identity |
...
| `src/fheroes2/agent/agent_capabilities.{h,cpp}` + `python/fheroes2_agent/data/monster_capabilities_v1.json` | Machine-generated 71-monster audit; action-space-based `simple_v1` allowlist; scenario profile gate; raw per-monster ability and weakness records (type id, percentage, value), the guide's layer-1 representation, additive as of 2026-08-05; the layer-2 semantic adapter (`classifyAbility`/`classifyWeakness`, additive as of 2026-08-07) stamping every record with closed-vocabulary `trigger`/`target`/`effect`/`magnitude_kind`, each mapping citing its engine call site, per [[observation-design#Ability records, three layers, two built]] | §4.2, §11.1 | `test_agent_capabilities` (23 checks); regeneration byte-compares previous fields |
...
| `agent_play/verify_m3.sh` | M3 gate: 9 checks, unit tests, determinism+coverage verdict, golden digests under enumeration, 100 % coverage, min-candidates, cross-process byte-identity, audit sanity | §20-M3 | self |
...
| `src/fheroes2/agent/agent_external_controller.{h,cpp}` | Blocking decision callback so an outside policy answers a turn; controllable side, rejection and end-of-input both fall back to skip; opt-in teacher probe asks the planner's `queryUnitTurn` at every answered decision for DAgger relabeling | §5.4 | `test_protocol.py` (22 checks) + `planner_query.py` digest pairs |
...
- M4: strict scenario JSON parsing that names a path and a stable error code on rejection, a vendored JSON parser per §6.5, the `observable_v1` profile per ADR 0001 (the `planes_v1` modality of ADR 0004 landed 2026-08-06/07, see the component row above), an `ENABLE_AGENT` CMake target, and `verify_m4.sh` itself. Blocking external control is done, and the current line protocol is a working subset of §13 rather than protocol v1.
