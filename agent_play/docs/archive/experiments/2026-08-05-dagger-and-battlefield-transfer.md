...
On side swapping: everything ever trained controlled the attacker, and the first measurement overstated the gap. As defender on the held-out pool the agent wins 0.252 against 0.567 as attacker, but those matchups hand the defender a different army, one calibrated to be beatable by the attacker, so the comparison confounds side skill with army quality; the honest signal in it is that training improved defender play by only $+0.017$ where it improved attacker play by $+0.146$.

The owner's symmetry argument prompted the clean test, mirror armies played from each side, and it dissolved the simple story: per-composition swings dominate (0.042 as attacker against 0.667 as defender on an elf-heavy mirror, 0.250 against 0.000 on a cavalry one, means 0.174 attacker and 0.368 defender over six mirrors). Shooter-heavy compositions structurally favor the passive side, so left-right symmetry holds only modulo first-move and composition effects plus asymmetric obstacles; an optimal policy is side-symmetric in the encoding's sense, but the game itself is not mirror-neutral. Both-side training stays warranted, with the capture pipeline's per-side model routing as the mechanism. Stochasticity got its mechanical answer on the way: the scenario seed pins the engine's generator only, while action sampling draws from the process's own torch generator, seeded once and never reset per episode, so eight free-running episodes of one battlefield produced eight distinct trajectories and re-seeding torch reproduced one bit-identical episode.
