...
Stage 2 exists, its first round is measured, and the shape after the corrected arms is: teacher-state data generalizes teacher coverage, student-state relabeling repairs student drift, the two combine into the best pool generalist so far, and mixture weight decides which signal survives where their states overlap, with the extreme-horde rung the visible casualty. The recorded next steps, in the order the evidence ranks them: iterate DAgger from the combination clone, since round one returned this much from the weaker v4; treat mixture weighting as an explicit axis rather than an accident, with the relabeling share swept where today it was only dropped from two thirds to zero; and only then reinforcement learning from the strongest clone. The difficulty-weighted reward and battlefield rotation stay available and measured-null at current scales.
