...
Two signals separate cleanly on the first seed. Most of the play-level gain is capacity: the width control leads held-out, the ladder, and commanders, which is exactly the confound the third arm was built to expose and why no two-arm version of this experiment would have been honest. And the board information itself shows in the two places it mechanically should: cloning agreement, where planes beat the width control by 1.9 points (the teacher's moves are more predictable when you can see what it walks around), and attacker-side mirrors, where symmetric armies cancel composition and position is the only edge, 0.278 against 0.194 against 0.090. Reports `planes_ablation.json`, `battery_planes.json`.

## The replications, and what each thread earned

Seeds one and two of both experiments ran the same evening, after one relaunch: the original chains died on a worker binary pinned before the planes build, whose battery rejected `--planes`, which is now a recorded convention (pin workers per feature, not per day).

The planes fidelity signal replicates cleanly. Cloning agreement over three seeds: planes $0.897 \pm 0.002$, width control $0.875 \pm 0.004$, entity $0.846 \pm 0.001$; the planes-over-width gap is five to ten times the seed noise, so seeing the board makes the teacher genuinely more predictable, beyond any capacity story. Play-level effects are real but modest: planes lead held-out ($0.533 \pm 0.022$ against the control's $0.513 \pm 0.048$) and commander extremes, the two arms tie on attacker mirrors with both far above entity, and the width control keeps the defender suites. Unlike the ability arm, planes collapse no flagship. The next planes question is recorded rather than run: the trio trained on the planes corpus alone, so the champion-mixture version needs either obstacle layers recorded for the older corpora or an explicit unknown-board convention for mixed training.

The soft-target tilt does not replicate: held-out $+0.04/+0.02/-0.02$ across seeds, the ladder and defender mirrors with the argmax twin throughout. The thread closes as measured-negative at this entropy, the survey carries the full account, and the per-candidate recording stays for the value-on-search-returns line.
