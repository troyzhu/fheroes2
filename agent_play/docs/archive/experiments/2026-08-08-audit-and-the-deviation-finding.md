...
## Retraining under commander parity, and why its verdict is not clean

The owner asked for the training redone with the commander priced. The corpus was recollected on matchups from the repriced budget sampler, screened the same way, with search scoring the commander-aware reward, and distilled with the same regret weighting at three seeds. It kept 16 of 54 matchups and 2,640 labels.

Against the regret-band arm it trades rather than settles anything. The Thunk ladder improves, 0.771 against 0.708 with the reward column agreeing at 1.220 against 1.033 and win quality 0.652 against 0.514, while held-out falls to 0.546 from 0.571 and the defender mirror to 0.292 from 0.375. Normalized entropy and effective action count sit close on both, 0.21 and 2.6 against 0.19 and 2.3 on held-out, so the two arms are about equally decided.
...
The marginal returns are not smooth: four to eight simulations buys $+0.017$ for 0.82 seconds, eight to sixteen buys $+0.117$ for 1.60, and sixteen to thirty-two buys $+0.042$ for 3.04.

The first explanation written here for that shape was wrong and is retracted. It attributed the eight-to-sixteen jump to the coverage-forced sweep consuming small budgets, but the playing path does not sweep: `search_action_detail` defaults `coverage_forced` to false and `search_agent_battery.py` calls `search_action` without it, so coverage forcing has only ever applied to the label collector. Anyone reading the sweep-truncation curve against this ladder is reading the wrong curve.

The control law that does apply is a first-success scan, and it predicts the shape without a threshold. Every candidate's value starts at zero and both reward margins are sign-separated, a win returning at least $+1$ and a loss at most zero, so the first simulation always goes to the prior's argmax and, if that rollout wins, the incumbent's score is unassailable by an unvisited rival at any plausible exploration constant. Budget therefore buys nothing on the decisions where the prior's top move already wins its playout, which is most of them, and buys everything on the rest, where the agent has to find a winner among lower-prior candidates and can try about as many as the budget allows. That predicts a curve concave in the logarithm of the budget with no knee, which is what the rungs show.
