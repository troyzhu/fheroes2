...
| `experiments/2026-08-10-search-configuration-and-two-retractions.md` | The three search settings nobody had chosen, the dice leakage that retracted a nine-suite win into a tie, and the winner's-curse control that turned a damning teacher comparison into a label-noise finding | Conclusions live in [[../rl/reward-design|reward-design]] |
| `experiments/2026-08-11-distillation-budget-and-checkpoint-selection.md` | Why a cosine-annealed run cannot evidence its own convergence, the fifty-epoch gap between the two offline selectors, and the ladder on which held-out agreement runs $-0.436$ against play | Conclusions live in [[../rl/training-design|training-design]] and [[../rl/program-review|program-review]] |
| `experiments/files/` | Raw run artifacts a log depends on: replay recordings, dated run-report and recording-manifest snapshots, and the anchor checkpoints pools and recordings are calibrated against. Each dated directory carries a generated `README.md` grouping its files by family, since a directory of a hundred and more JSONs is provenance rather than a reading surface; `agent_play/experiments/index_run_reports.py` writes them and the gate fails on a file no family explains | Meaningful only through the logs that cite them; datasets re-record from their manifests |
