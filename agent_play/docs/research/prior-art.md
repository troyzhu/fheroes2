...
## Training from nothing, the genre's verified record

The owner asked on 2026-08-08 whether open-source projects have trained battle policies from scratch, no human data and no demonstrations, in games like ours, and two verification passes over the repositories and competition records give a sharp answer: yes, under conditions, and the conditions are consistent enough to read as a law. The full per-source evidence lives in [[works/vcmi-gym]], [[works/gym-microrts]], [[works/lux-ai-competitions]], [[works/pokemon-selfplay-from-scratch]] and [[works/botbowl-competitions]].

The verified successes are vcmi-gym, from-scratch masked PPO on HoMM3 battles reaching 72 percent against the strong scripted AI and shipping inside VCMI 1.7 after roughly three years of single-machine iteration; gym-microrts and its competition descendant RAISocketAI, the first deep-learning winner of that competition; Lux season one's winner, pure self-play on a personal machine; and the 2019 Pokémon result, self-play PPO beating a tree-search bot for about ninety dollars of compute. Every one of them combines hard invalid-action masking, a spatial convolutional policy and value, and a densely shaped reward, with the strongest recipes annealing the shaping away toward sparse win-loss as competence grows. Opponent structure splits by ecology: where a strong scripted incumbent exists the successes train against it directly, and the microrts paper measured pure self-play as worse; where none exists, self-play with a frozen-teacher bridge carried Lux. None of them needed league play or industrial throughput; every success fits one to three GPUs.

The verified negatives draw the boundary. Blood Bowl's four competitions never saw strictly-from-scratch reinforcement beat a scripted bot, a random agent scoring zero touchdowns in 350,000 matches, and the one machine-learning win required an imitation bootstrap; Pokémon's full six-versus-six format defeats the same self-play recipe that wins the small format until it is imitation-initialized; Lux season two was won by a rule-based bot over every learned entry with a GPU engine freely available; Wesnoth, Stratega and the card-battlers have no measured from-scratch success at all. The failure mode the negatives share is a reward chain too long for random play to reach, and the successes' shaping exists precisely to shorten it.

For this project the record reads three ways. Our tabula-rasa pilot failed exactly on the genre's fault line, rollout-scored search over a random prior has no reachable signal, and the fix the genre used is dense shaping we deliberately rejected in [[../rl/reward-design]] in favor of an imitation start, the other working corner of the same trade, the one botbowl's and the six-versus-six Pokémon winners landed on. Second, our current phase already matches the ecology rule: a strong scripted incumbent exists here, and the pool trains against it as anchor. Third, if a from-scratch arm is ever wanted for its own sake, the genre's recipe is written: shaped events annealed toward the owner objective, the built-in AI as primary opponent, planes observation, masking already in place, and hobby-hardware budgets suffice.
