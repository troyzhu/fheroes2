...
The modern descendant is PUCT as used by AlphaZero ([[alphazero]]), which replaces the log bonus with a prior-weighted one, $Q(a) + c \, P(a) \sqrt{N} / (1 + n_a)$, so a learned policy focuses the search. That is the form `search_probe.py` implements at the root, with rollout returns for $Q$ and the clone as $P$.
