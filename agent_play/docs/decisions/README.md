...
| [[0008-search-configuration]] | What root search maximizes, which battlefield and which dice its side environment gets, whether coverage is forced, and that a suite verdict needs seeded repeats; four settings that had never been chosen and each worth more than most tuning | extends 0005's operator set | 2026-08-10 | Yes, `fheroes2_agent/search.py` and the scenario's `combatSeedOffset` |
