...
Built and measured on 2026-08-03. Explained variance moves from $-3.061$, worse than predicting the mean, to $+0.835$, and the value head must be frozen against the shared trunk or teacher agreement falls from 0.887 to 0.701. It does not measurably improve stage 3: over 60 cold and 35 pre-fitted seeds the paired difference in final win rate is $+0.033 \pm 0.027$, on a matchup both arms solve every time. The stage stays as written, because a better critic is cheap and correct and the measurement so far cannot separate a small benefit from none, but it is not yet earning its place and the record should not claim it is.
