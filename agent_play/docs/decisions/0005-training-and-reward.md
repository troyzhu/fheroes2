...
Stage 2b, critic pre-fitting. The teacher plays both sides of every episode, so realized returns are available in unlimited quantity at roughly 4,600 episodes per second, and regressing a value head on them fits $V^{\pi^{*}}$ by Monte Carlo policy evaluation before any reinforcement learning starts. This costs nothing that is not already being generated and removes the uninformative-critic window at the start of stage 3, which is when the cloned policy is most exposed. It needs the reward settled first, though not new data, since the recorded terminal state supports computing returns retroactively. [[../rl/training-design#Pre-fitting the critic on teacher play]] gives the argument, including why the mismatch between $V^{\pi^{*}}$ and $V^{\pi_\theta}$ leaves the gradient unbiased.
