...
| `selfplay_search_round.py` | One expert-iteration round in true self-play: search over policy-vs-policy rollouts labels both chairs of policy-vs-policy games, honest dice, record-only margins; the curriculum stage after the AI-opponent rounds | about 30 min collection at 24 matchups |
...
| `search_objective.py` | Which terminal reward root search should maximize, since `rollout` returns the side environment's reward and the choice was never made deliberately; every margin over the mirror suite from both chairs, paired per battlefield | about 45 min for four margins |
| `search_sync.py` | Whether the search side environment must sit on the same battlefield as the live battle, since `rollout`'s prefix replay only reproduces the live state when it does; every harness shared the live `seeds` and searched variant zero while the battle rotated | about 15 min |
| `search_leakage.py` | Whether the corrected search is planning or reading the live battle's dice: pinning the side environment to the live world seed fixes the terrain but also inherits the combat stream, and this separates the two with a combat-seed offset | about 45 min for three arms |
| `search_strength.py` | What buys root search strength once it can no longer read the live dice: simulation budget, PUCT exploration weight and coverage forcing, measured against each other with the candidates actually visited reported beside the win rate | about 20 min per cell grid |
| `teacher_leakage.py` | Whether knowing the dice changes what the search teacher labels, asked at identical states so it compares choices rather than playthroughs; decides whether every corpus on record needs re-collecting | about 25 min |
| `label_value.py` | Whether a search overrule is actually better than the policy's own choice, by replaying the battle many times from each branch on independently drawn dice; the causal test that individual-label noise does not settle | about 60 min |
...
| `report_summary.py` | Prints the canonical multi-metric block for any report card, from either harness and from any number of seeded repeats of one configuration, averaged per matchup with the seed spread beneath. The one place a summary should come from, because a summary written by hand collapses to the win rate | seconds |
...
Adding a column to a harness means adding it to the aggregation too, or it never reaches the report. `search_agent_battery.py` collected six columns and aggregated a hardcoded list of six, so the entropy, support, rounds and reward-split columns `scenarios.measure` had carried since 2026-08-09 were absent from every searched arm and a searched arm could only ever be compared to a policy arm on the rate. The aggregation now walks whatever numeric keys the episodes produced. Entropy is reported in both forms, raw in nats and normalized against the uniform maximum over the state's own legal set, and a searched arm additionally reports the entropy of its own visit distribution, which says whether the budget resolved anything rather than how decided the network was.

The deployment rule belongs to any harness that plays, not just the policy battery. It had been a flag on `validation_battery.py` alone since the deployment sweep settled the action rule, so every searched number was implicitly sampled, even though the rule governs the search prior as well as the played move and a sharper prior changes which candidates PUCT visits at all.

One flag, one job. `search_agent_battery.py` passed a single `--reward-margin` to both the live environment, which decides what the `rw` column measures, and the side environment, which decides what search maximizes, while the report went on stamping `two_sided` regardless. Asking for a search objective therefore changed the reported reward silently and the stamp then lied about it, which is the same defect the 2026-08-08 rule was written against, one level up. The two are now `--report-margin` and `--reward-margin` and both are stamped.

The comparison that mistake produced is worth remembering: the built-in AI baseline hardcodes the two-sided reward, so a run reporting the hit-point margin was being set beside it on a different scale. When two reward columns meet, check that both name the same objective.

A suite mean is only worth reading if the suite can move. Counting matchups where every arm and the engine sit at the identical extreme, `real_maps` is 22 of 24 saturated, `stress_hordes` 4 of 5, `stress_commanders` 3 of 4 and the Thunk ladder 2 of 4, so at most 8, 20, 25 and 50 percent of those means can respond to any policy change at all. `real_maps` reading 0.569 for the searching agent, 0.568 for the unsearched network and 0.568 for the engine is not a coincidence and not evidence of parity: it is a near-constant, on battles averaging 4.6 decisions. The battery prints a decided-matchup count beside every suite for this reason, and a summary that filters that line out is hiding the thing that says whether the row means anything. Quote the live suites, quote the ladder by rung, and say which is which.
...
A seed is only a seed if it is stable across processes. The first version of the battery's per-matchup seeding salted the seed with `hash(suite)`, and Python randomizes string hashing per process unless `PYTHONHASHSEED` is pinned, so every run drew a different stream and the runs labelled with a seed were not reproducible. It surfaced when one configuration re-read 0.750 where the same seed had read 0.675. `zlib.crc32` replaces it. The measurements taken under the unstable salt remain valid as independent samples, which is what the spread estimate used them for, and are not reproducible configurations, which is what they were labelled as.

A harness that samples must be seeded, and a single run of one is not a measurement. `search_agent_battery.py` called no `torch.manual_seed` until 2026-08-10, so the policy sampling inside every playout began from inherited process state, while the standard errors it printed described spread across matchups within one run. Two runs of one configuration on held-out read 0.619 and 0.750. Seeded per matchup, five repeats read 0.656 to 0.725, a standard deviation of 0.031, and both unseeded readings fall outside that range. Seed per matchup rather than once per run, so a suite's numbers do not depend on which suites preceded it, and quote the between-run spread beside any suite verdict.

What a search maximizes is a configuration, not a property of the method. `rollout` returns the side environment's terminal reward, so that environment's `reward_margin` is the objective, and passing nothing inherits a default that has no reason to be right. Measured over the mirror suite, the survival-graded margins beat the destruction-graded ones by about $+0.19$, which is larger than most effects this project chases.

A side environment pinned to the live world seed shares the live battle's dice as well as its battlefield, because the combat seed derives from the map seed. The pinning is required, since the prefix replay reproduces the live position only on the right battlefield, but without a `combat_seed_offset` the search evaluates candidates under the rolls that will actually happen. That is worth $+0.323$ and it is a ceiling rather than a score. Pass the offset for any number quoted against an opponent that gets no model, and say which was used either way.

Any claim that one search's choices are worse than another's needs a same-information control arm. A search picks the argmax of its own estimates, so every other action it scored is below that by construction, and comparing two searches produces a large negative from the winner's curse alone. The first teacher comparison read $-0.48$ and looked damning; a second identically-informed teacher read $-0.5721$ against the first's $-0.5724$, so the true difference was $-0.0003$. Without the control the artifact is indistinguishable from the effect.
