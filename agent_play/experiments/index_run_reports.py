...
FAMILIES: list[tuple[str, str, str]] = [
    ...
    ("push45_*", "search_agent_battery.py with the live dice",
     "The same sweep with the side environment sharing the live world seed, so search saw the rolls the\n      battle was about to make; a ceiling rather than a score, kept because the retraction cites it"),
    ("battery_noforce*", "validation_battery.py",
     "Distillation from a corpus collected without coverage forcing against the forced corpus, three\n      paired seeds at matched soft mass; confident labels that net nothing and trade axes instead"),
    ("battery_margin*", "validation_battery.py",
     "The confidence-filter distillation arms, with and without the single-playout tie-break overrules,\n      three paired seeds each; the arm that measured negative and was not adopted"),
    ("deploy_search_*", "search_agent_battery.py --deployment",
     "The action rule crossed with search for the first time, with the full column block including\n      both entropy forms and the search's own visit entropy; shows greedy starving the search"),
    ("spread_s*", "search_agent_battery.py --seed, one suite repeated",
     "How far one battery number moves between runs of an identical configuration, which nothing had\n      measured because the harness seeded nothing; the error bar every single-run search figure lacked"),
    ("fullmetrics_s*", "search_agent_battery.py with the full column block",
     "The goal-45 scoreboard with every column the harness can now report, three seeds and a paired\n      unsearched arm; the version that replaces the rates-only sweep beside it"),
    ("final_sweep_*", "search_agent_battery.py, every correction applied",
     "The goal-45 scoreboard with the survival-graded objective, the live battlefield, independent dice\n      and a seed, three of them, which is the first version of this table with a real error bar"),
    ("search_strength*", "search_strength.py",
     "Simulation budget against PUCT exploration weight against coverage forcing, with the candidates\n      each cell actually visited, which is what shows forcing spending a budget on breadth"),
    ("honest_ladder_*", "search_agent_battery.py at rising budgets",
     "The simulation ladder redone with the side environment on the live battlefield and independent\n      dice; the published ladder had neither and every rung of it was an upper bound"),
    ("label_value*", "label_value.py",
     "Whether a search overrule actually beats the policy's own choice, by replaying the battle from\n      each branch on independently drawn dice; the causal test that label-noise measurements cannot settle"),
    ("teacher_leakage*", "teacher_leakage.py",
     "Whether knowing the dice changes what the search teacher labels, asked at identical states, with a\n      second same-information teacher as the winner's-curse control that the first run lacked"),
    ("terrain_probe*", "the mirror terrain probe",
     "Identical armies on six battlefields, isolating how much of a matchup's win rate is the obstacle\n      layout rather than the armies"),
    ...
    ("parity_*", "search_teacher.py under commander-priced sampling",
     "A corpus collected on matchups whose budget prices the commander, with search scoring the\n      commander-aware reward, and the distillation arms trained on it"),
